...
local Title_Name = "Talent Viewer"
...
btnClear:SetPoint("LEFT", btnCache, "RIGHT", 6, 0)
...
StatFrame:SetPoint("BOTTOMLEFT", F, "BOTTOMRIGHT", -5, 40)
...
local function GetStatRow(index)
    ...
    f:SetScript("OnEnter", function(self)
        ...
        local s = {}
        for n, v in pairs(self.sources) do table.insert(s, {n=n, v=v.val, desc=v.desc}) end
        table.sort(s, function(a,b) return a.v > b.v end)
        for _, d in ipairs(s) do
            local display = d.desc or d.v
            local suffix = self.statName:find("%%") and "%" or ""
            GameTooltip:AddDoubleLine("|cffffffff"..d.n.."|r", "|cff00ff00+"..display..suffix.."|r")
        end
        GameTooltip:Show()
    end)
...
local function UpdateStatSummary()
    local totals, sources = {}, {}
    
    -- Ensure the external data exists
    ...
    for name, data in pairs(MiniTalentCacheChar) do
        ...
        if talentID and CL_FixedTalents[talentID] then
            for _, stat in ipairs(CL_FixedTalents[talentID]) do
                ...
                local descVal = val -- Store for tooltip display

                -- Handle Special Calculations
                if val == "ARMOR_CALC" then
                    local _, armor = UnitArmor("player")
                    val = math.floor(armor / 108) * 3
                    descVal = val
                end

                local finalLabel = label .. (isPct and " %" or "")
                totals[finalLabel] = (totals[finalLabel] or 0) + val
                
                sources[finalLabel] = sources[finalLabel] or {}
                sources[finalLabel][name] = {val = val, desc = descVal}
            end
        end
    end
